...
$$"
                ...
                {{-- Thumbnail hiện tại --}}
                @if($post->thumbnail)
                <div class="flex flex-col gap-y-2 mb-4" id="current-thumbnail-section">
                    <x-labels.label>Ảnh đại diện hiện tại</x-labels.label>
                    <div class="relative inline-block w-48">
                        <img 
                            src="{{ asset('storage/' . $post->thumbnail->path) }}" 
                            alt="{{ $post->title }}" 
                            class="w-48 h-auto border border-accent rounded"
                            id="current-thumbnail-img"
                        />
                        <button 
                            type="button" 
                    ...
                    <small class="text-sm text-gray-400">Click vào X để xóa ảnh này</small>
                </div>
                @endif
                ...
                <div class="flex flex-col gap-y-2 mb-4">
                    ...
                    <div class="border border-accent p-2 rounded min-h-[42px] cursor-text" id="tags-container">
                        ...
                        <input 
                            ...
                            autocomplete="off"
                        />
                        <input type="hidden" name="tags" id="tags-hidden" value="{{ old('tags', $post->tags->pluck('name')->implode(', ')) }}">
                    </div>
                ...
                <div class="border border-accent p-4 mb-4">
                    ...
                    <div class="flex flex-col gap-y-2 mb-4">
                        ...
                        <div class="border border-accent p-2 rounded min-h-[42px] cursor-text" id="meta-keywords-container">
                            ...
                            <input 
                                ...
                                autocomplete="off"
                            />
                            <input type="hidden" name="meta_keywords" id="meta-keywords-hidden" value="{{ old('meta_keywords', $post->seoMeta->meta_keywords ?? '') }}">
                        </div>
                    ...
                    <div class="flex flex-col gap-y-2 mb-4" id="current-og-image-section">
                        <x-labels.label>OG Image hiện tại</x-labels.label>
                        <div class="relative inline-block w-64">
                            <img 
                                src="{{ asset('storage/' . $post->seoMeta->ogImage->path) }}" 
                                alt="OG Image" 
                                class="w-64 h-auto border border-accent rounded"
                                id="current-og-image-img"
                            />
                            <button 
                                type="button" 
                                id="remove-current-og-image"
                                class="absolute top-2 right-2 bg-red-600 text-white p-2 rounded-full hover:bg-red-700 shadow-lg"
                                title="Xóa ảnh hiện tại"
                            >
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <input type="hidden" name="remove_og_image" id="remove-og-image-flag" value="0">
                        <small class="text-sm text-gray-400">Click vào X để xóa ảnh này</small>
                    </div>
                    @endif
                ...
                <div class="flex items-center gap-4">
                    ...
                    <button type="submit" form="delete-post-form-{{ $post->id }}" class="ml-auto py-2 px-4 bg-red-600 text-white hover:bg-red-700 rounded" onclick="return confirm('Bạn có chắc chắn muốn xóa bài viết này?');">
                        Xóa bài viết
                    </button>
                </div>
            </form>
            ...
            <form id="delete-post-form-{{ $post->id }}" action="{{ route('admin.posts.destroy', $post->id) }}" method="POST" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        </x-cards.card>
    ...
    <script>
        ...
        const currentThumbnailSection = document.getElementById('current-thumbnail-section');
        const removeThumbnailFlag = document.getElementById('remove-thumbnail-flag');
        
        if (removeCurrentThumbnailBtn) {
            removeCurrentThumbnailBtn.addEventListener('click', () => {
                if (confirm('Bạn có chắc chắn muốn xóa ảnh đại diện hiện tại?')) {
                    // Add visual feedback
                    const img = document.getElementById('current-thumbnail-img');
                    img.style.opacity = '0.3';
                    img.style.filter = 'grayscale(100%)';
                    
                    // Show removed badge
                    const badge = document.createElement('div');
                    badge.className = 'absolute inset-0 flex items-center justify-center bg-black/50 rounded';
                    badge.innerHTML = '<span class="text-white font-bold text-sm">Sẽ bị xóa</span>';
                    img.parentElement.appendChild(badge);
                    
                    // Hide remove button
                    removeCurrentThumbnailBtn.style.display = 'none';
                    ...
                    restoreBtn.className = 'mt-2 text-sm text-accent hover:text-accent/70';
                    restoreBtn.innerHTML = 'Hoàn tác';
                    restoreBtn.onclick = () => {
                        img.style.opacity = '1';
                        img.style.filter = 'none';
                        badge.remove();
                        restoreBtn.remove();
                        removeCurrentThumbnailBtn.style.display = 'block';
                        removeThumbnailFlag.value = '0';
                    };
                    currentThumbnailSection.appendChild(restoreBtn);
                }
            });
        }

        // Remove current OG image
        const removeCurrentOgImageBtn = document.getElementById('remove-current-og-image');
        const currentOgImageSection = document.getElementById('current-og-image-section');
        const removeOgImageFlag = document.getElementById('remove-og-image-flag');
        
        if (removeCurrentOgImageBtn) {
            removeCurrentOgImageBtn.addEventListener('click', () => {
                if (confirm('Bạn có chắc chắn muốn xóa OG Image hiện tại?')) {
                    // Add visual feedback
                    const img = document.getElementById('current-og-image-img');
                    img.style.opacity = '0.3';
                    img.style.filter = 'grayscale(100%)';
                    ...
                    // Hide remove button
                    removeCurrentOgImageBtn.style.display = 'none';
                    
                    // Set flag
                    removeOgImageFlag.value = '1';
                    ...
                    restoreBtn.onclick = () => {
                        ...
                        badge.remove();
                        restoreBtn.remove();
                        removeCurrentOgImageBtn.style.display = 'block';
                        removeOgImageFlag.value = '0';
                    };
                    currentOgImageSection.appendChild(restoreBtn);
                }
            });
        }
    </script>
